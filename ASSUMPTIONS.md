...
**8. Batch without application records = triage, not full matching.**
A pile of images alone cannot be field-matched (there is nothing to match against). Batch mode checks mandatory-element presence and warning exactness, which is precisely the routine screening Sarah's team does first. The demo batch (the real registry examples) demonstrates full matching because their application records are known.
